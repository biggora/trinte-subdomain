        ...
        if (host === options.base
            ...
            if(options.except) {
                if (new RegExp(options.except + '\.' + options.base,'i').test(host)) {
                    return exceptCallback ? exceptCallback(request, response, next) : next();
                }
            }
